...
newLine = os.linesep         
reJSscript = b'<script[^>]*?contentType\s*?=\s*?[\'"]application/x-javascript[\'"][^>]*?>(.*?)</script>'
...
def isJavascript(content):
    ...
    '''
    jsStrings = [b'var ', b';', b')', b'(', b'function ', b'=', b'{', b'}', b'if ', b'else', b'return', b'while ', b'for ', b',', b'eval']
    keyStrings = [b';', b'(', b')']
    reVarInit = b'var [\w0-9]+\s*?='
    reFunctionCall = b'[\w0-9]+\s*?\(.*?\)\s*?;'
    stringsFound = []
    limit = 15
    ...
    for char in content:
        print(type(content))
        if (char < 32 and chr(char) not in ['\n', '\r', '\t', '\f', '\x00']) or char >= 127:
            return False
