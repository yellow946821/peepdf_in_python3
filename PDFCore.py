...
MAL_BAD_HEAD = 6
pdfFile = None
newLine = bytes(os.linesep , encoding='utf8')
isForceMode = False
isManualAnalysis = False
spacesChars = [b'\x00',b'\x09',b'\x0a',b'\x0c',b'\x0d',b'\x20']
delimiterChars = [b'<<',b'(',b'<',b'[',b'{',b'/',b'%']
monitorizedEvents = [b'/OpenAction ',b'/AA ',b'/Names ',b'/AcroForm ', b'/XFA ']
monitorizedActions = [b'/JS ',b'/JavaScript',b'/Launch',b'/SubmitForm',b'/ImportData']
monitorizedElements = [b'/EmbeddedFiles ',
                       b'/EmbeddedFile',
                       b'/JBIG2Decode',
                       b'getPageNthWord',
                       b'arguments.callee',
...
jsVulns = [b'mailto',
           ...
           b'media.newPlayer',
           b'doc.printSeps',
           b'app.removeToolButton']
singUniqueName = b'CoolType.SING.uniqueName'
bmpVuln = b'BMP/RLE heap corruption'
...
class PDFHexString (PDFObject) :
    ...
    def getEncryptedValue(self):
        if type(self.rawValue) == bytes:
        	self.rawValue = self.rawValue.decode("utf-8") 
        return '<'+self.rawValue+'>'
        ...
        for i in range(len(keys)):
            ...
            if type in ['string','hexstring','array','dictionary'] and self.encrypted and not decrypt:
                ...
                if ret[0] == -1:
                    errorMessage = 'Error encrypting element'
                    self.addError(errorMessage)
            self.encryptedValue += bytes(rawValue , encoding='utf8') + b' ' + bytes(str(valueObject.getEncryptedValue()) , encoding='utf8') + newLine
            self.rawValue += bytes(rawValue , encoding='utf8') + b' ' + bytes(str(valueObject.getRawValue()) , encoding='utf8') + newLine
            self.value += bytes(keys[i] , encoding='utf8') + b' ' + bytes(v , encoding='utf8') + newLine
        self.encryptedValue = self.encryptedValue[:-1] + b' >>'
        self.rawValue = self.rawValue[:-1] + b' >>'
...
class PDFStream (PDFDictionary) :
        ...
        for i in range(len(keys)):
            ...
            if type in ['string','hexstring','array','dictionary'] and self.encrypted and not decrypt:
                ...
                if ret[0] == -1:
                    ...
                    self.addError(errorMessage)
            self.encryptedValue += bytes(rawValue , encoding='utf8') + b' ' + bytes(str(valueElement.getEncryptedValue()) , encoding='utf8') + newLine
            self.rawValue += bytes(rawValue , encoding='utf8') + b' ' + bytes(str(valueElement.getRawValue()) , encoding='utf8') + newLine
            self.value += bytes(keys[i] , encoding='utf8') + b' ' + bytes(v , encoding = 'utf8') + newLine
        self.encryptedValue = self.encryptedValue[:-1] + b' >>'
        self.rawValue = self.rawValue[:-1] + b' >>'
        self.value = self.value[:-1] + b' >>'
                ...
                if self.deletedFilters:
                    ...
                    if not self.updateNeeded:
                        ...
                        if not self.isFaultyDecoding():
                            refs = re.findall(b'(\d{1,5}\s{1,3}\d{1,5}\s{1,3}R)', self.decodedStream)
                            if refs != []:
                                self.references += refs
                                self.references = list(set(self.references))
                            if isJavascript(self.decodedStream) or self.referencedJSObject:
    ...
    def getValue(self):    
        ...
        return bytes(self.value,encoding='utf8') + newLine + b'stream' + newLine + bytes(self.decodedStream , encoding='utf8') + newLine + b'endstream'
...
class PDFTrailer :
    ...
    def update(self, streamPresent = False):
        ...
        if self.dict == None:
            ...
            self.addError(errorMessage)
            return (-1,errorMessage)
        if self.dict.hasElement(b'/Root'):
            reference = self.dict.getElementByName(b'/Root')
            if reference != None:
...
class PDFParser :
    def __init__(self) :
        self.commentChar = '%'
        self.comments = []
        self.delimiters = [(b'<<',b'>>',b'dictionary'),(b'(',b')',b'string'),(b'<',b'>',b'hexadecimal'),(b'[',b']',b'array'),(b'{',b'}',b''),(b'/',b'',b'name'),(b'%',b'',b'comment')]
        self.fileParts = []
        self.charCounter = 0    
            ...
            if rawIndirectObjects != []:
                for j in range(len(rawIndirectObjects)):
                    ...
                    while True:
                        ...
                        if not re.match(b'\d{1,10}'+objectHeader,checkHeader):
                            break
                        else:
                    ...
                    if ret[0] != -1:
                        ...
                        if pdfIndirectObject != None:
                            ...
                            if pdfObject != None:
                                objectType = pdfObject.getType()
                                if objectType == b'dictionary':
                                    if isFirstBody and not linearizedFound:
                                        if pdfObject.hasElement(b'/Linearized'):
                                            pdfFile.setLinearized(True)
                                            linearizedFound = True
                                elif objectType == b'stream' and type == b'/XRef':
    ...
    def createPDFIndirectObject (self, rawIndirectObject, forceMode = False, looseMode = False) :
        ...
        try:	
            ...
            print('alive5')
            ret = self.readSymbol(rawIndirectObject, b'obj')
            print('alive6')
            

            if ret[0] == -1:
                return ret
            rawObject = rawIndirectObject[self.charCounter:] ##bytes
            print('alive7')
            ...
            object = ret[1]
            pdfIndirectObject.setObject(object)
            ret = self.readSymbol(rawIndirectObject, b'endobj', False)
    ...
    def createPDFDictionary(self, rawContent):
        ...
        elements = {}
        rawNames = {}
        ret = self.readObject(rawContent[self.charCounter:], b'name')
        if ret[0] == -1:
            if ret[1] != 'Empty content reading object':
                if isForceMode:
                    pdfFile.addError(ret[1])
        ...
        while name != None:
            ...
            if ret[0] == -1:
                if isForceMode:
                    ...
                    ret = self.readUntilSymbol(rawContent, b'/')
                    if ret[0] == -1:
    ...
    def createPDFStream(self, dict, stream):
        ...
        elements = {}
        rawNames = {}
        ret = self.readObject(dict[self.charCounter:], b'name')
        if ret[0] == -1:
            if ret[1] != 'Empty content reading object':
                if isForceMode:
                    pdfFile.addError(ret[1])
    ...
    def createPDFCrossRefSection (self, rawContent, offset):
        ...
        if not isinstance(rawContent, bytes):
        ...
        beginSubSectionRE = re.compile(b'(\d{1,10})\s(\d{1,10})\s*$')
        entryRE = re.compile(b'(\d{10})\s(\d{5})\s([nf])')
        ret = self.readSymbol(rawContent, b'xref')
        if ret[0] == -1:
            return ret
    ...
    def createPDFTrailer (self, rawContent, offset, streamPresent = False) :
        ...
        self.readSymbol(rawContent, b'trailer')    
        ret = self.readObject(rawContent[self.charCounter:],b'dictionary')
        if ret[0] == -1:
            dict = PDFDictionary('')
            dict.addError('Error creating the trailer dictionary')
        else:
            dict = ret[1]
        ret = self.readSymbol(rawContent, b'startxref')
        if ret[0] == -1:
            try:
            ...
            try:
                ...
                return (-1, errorMessage)
        trailer.setOffset(offset)
        eofOffset = rawContent.find(b'%%EOF')
        if eofOffset == -1:
    ...
    def getIndirectObjects(self, content, looseMode = False):
        ...
        if not isinstance(content,bytes):
            return matchingObjects
        if not looseMode:
            regExp = re.compile(b'((\d{1,10}\s\d{1,10}\sobj).*?endobj)',re.DOTALL)
            matchingObjects = regExp.findall(content)
        else:
            regExp = re.compile(b'((\d{1,10}\s\d{1,10}\sobj).*?)\s\d{1,10}\s\d{1,10}\sobj',re.DOTALL)
    ...
    def readObject(self, content, objectType = None, forceMode = False, looseMode = False):
        ...
        '''
        global pdfFile
        if len(content) == 0 or content[:6] == b'endobj':
        ...
        for delim in delimiters:
            ...
            if ret[0] != -1:
                ...
                if delim[2] == b'dictionary':
                    ...
                    streamFound = re.findall(b'[>\s]stream', nonDictContent)
                    if streamFound:
                        ret = self.readUntilSymbol(content, b'stream')
                        if ret[0] == -1:
                            return ret
                        auxDict = ret[1]
                        self.readSymbol(content, b'stream', False)
                        self.readUntilEndOfLine(content)
                        self.readSymbol(content, b'\r', False)
                        self.readSymbol(content, b'\n', False)
                        ret = self.readUntilSymbol(content, b'endstream')
                        if ret[0] == -1:
                            stream = content[self.charCounter:]
                        else:
                            stream = ret[1]
                            self.readSymbol(content, b'endstream')
            ...
            if chr(content[0]) == 't' or chr(content[0]) == 'f':
                ret,raw = self.readUntilNotRegularChar(content)
                pdfObject = PDFBool(raw)
            elif chr(content[0]) == 'n':
                ret,raw = self.readUntilNotRegularChar(content)
                pdfObject = PDFNull(raw)
            elif re.findall(b'^(\d{1,10}\s{1,3}\d{1,10}\s{1,3}R)', content, re.DOTALL) != []:
                ret,id = self.readUntilNotRegularChar(content)
                ret,genNumber = self.readUntilNotRegularChar(content)
                ret = self.readSymbol(content, b'R')
                if ret[0] == -1:
                    return ret
                pdfObject = PDFReference(id, genNumber)
            elif re.findall(b'^([-+]?\.?\d{1,15}\.?\d{0,15})', content, re.DOTALL) != []:
    ...
    def readSpaces(self, string):
        ...
        for i in range(self.charCounter,len(string)):
            if bytes(chr(string[i]) , encoding='utf8') not in spacesChars:
                break
            self.charCounter += 1
        spacesCounter -= self.charCounter
    ...
    def readSymbol(self, string, symbol, deleteSpaces = True):
        ...
        if self.charCounter > len(string)-1:
            errorMessage = 'EOF while looking for symbol "'+str(symbol)+'"'
            pdfFile.addError(errorMessage)
            return (-1, errorMessage)
        while chr(string[self.charCounter]) == '%':
        ...
        if symbolToRead != symbol:
            errorMessage = 'Symbol "'+str(symbol)+'" not found while parsing'
            #pdfFile.addError(errorMessage)
            return (-1, errorMessage)
    ...
    def readUntilClosingDelim(self, content, delim):
        ...
        if numClosingDelims == 0:
            ...
            while indexChar != len(newContent):
                char = bytes(chr(newContent[indexChar]) , encoding='utf8')
                if indexChar == len(newContent) - 1:
                    nextChar = b''
                else:
                    nextChar = bytes(chr(newContent[indexChar+1]) , encoding='utf8')
                    #bytes(chr(char),encoding='utf8')
                if char == delim[1] or char + nextChar == delim[1]:
                    if char != b')' or indexChar == 0 or bytes(chr(newContent[indexChar-1]) , encoding='utf8') != b'\\':
                        return (0,output)
                    else:
                        output += char
                        indexChar += 1
                        self.charCounter += 1
                elif (char == b'(' and prevChar != b'\\') or (char in [b'[',b'<'] and delim[0] != b'('):
                    if (char + nextChar) != b'<<':
    ...
    def readUntilNotRegularChar(self, string):
        ...
        for i in range(self.charCounter,len(string)):    	
            if bytes(chr(string[i]) , encoding='utf8') in notRegChars:         	
                self.readSpaces(string)
                break
            readChars += chr(string[i])
            self.charCounter += 1
        return (0,readChars)
