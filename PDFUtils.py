...
def escapeString(string):
	...
	escapedValue = b''
	for i in range(len(string)):
		if bytes(chr(string[i]) , encoding='utf8') in toEscapeChars and (i == 0 or bytes(chr(string[i-1]) , encoding='utf8') != b'\\'):
			if bytes(chr(string[i]) , encoding='utf8') == b'\\':
				if len(string) > i+1 and re.match(b'[0-7]' , bytes(chr(string[i+1]) , encoding='utf8')):
					escapedValue += bytes(chr(string[i]) , encoding='utf8')
				else:
					escapedValue += b'\\' + bytes(chr(string[i]) , encoding='utf8')
			else:
				escapedValue += b'\\' + bytes(chr(string[i]) , encoding='utf8')
		elif bytes(chr(string[i]) , encoding='utf8') == b'\r':
			escapedValue += b'\\r'
		elif bytes(chr(string[i]) , encoding='utf8') == b'\n':
			escapedValue += b'\\n'
		elif bytes(chr(string[i]) , encoding='utf8') == b'\t':
			escapedValue += b'\\t'
		elif bytes(chr(string[i]) , encoding='utf8') == b'\b':
			escapedValue += b'\\b'
		elif bytes(chr(string[i]) , encoding='utf8') == b'\f':
			escapedValue += b'\\f'
		else:
			escapedValue += bytes(chr(string[i]) , encoding='utf8')
	return escapedValue
...
def unescapeString(string):
	...
	i = 0
	while i < len(string):
		if chr(string[i]) == '\\' and i != len(string)-1:
			if string[i+1] in toUnescapeChars:
				if string[i+1] == '\\':
					...
					pass
			elif chr(string[i+1]) == 'r':
				i += 1
				unescapedValue += '\r'
			elif chr(string[i+1]) == 'n':
				i += 1
				unescapedValue += '\n'
			elif chr(string[i+1]) == 't':
				i += 1
				unescapedValue += '\t'
			elif chr(string[i+1]) == 'b':
				i += 1
				unescapedValue += '\b'
			elif chr(string[i+1]) == 'f':
				i += 1
				unescapedValue += '\f'
			else:
				unescapedValue += chr(string[i])
		else:
			unescapedValue += chr(string[i])
		i += 1
